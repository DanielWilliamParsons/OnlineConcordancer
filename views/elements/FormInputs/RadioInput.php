...
class RadioInput {
	...
	public function concatenate($bool) {
		...
		$totalAttributes = $this->type .
									...
									$this->otherAttributes . 
									$checked . $disabled;
		$radio = $this->view->makeLinkWrap("input", $totalAttributes);
		if($bool === false) {
			return $this->view->makeTextWrap("label", "", $radio . $this->label); //Wraps the textbox in a label for the form.
		} else {
			return $this->view->makeTextWrap("label", "class='checkbox-inline'", $radio . $this->label); //Wraps the textbox in a label for the form.
		}
